...
- Policy creation is re-runnable. Postgres has no `create policy if not exists`, and re-running a migration is the natural recovery after a partial failure, so each policy is dropped first. Found by executing the migration twice, not by reading it.
...
- **The migration has been executed against a real PostgreSQL 16.13 database, not a Supabase branch.** Supabase branching requires the Pro plan and this project is on the free plan, so an isolated local Postgres was used instead, with `anon`, `authenticated` and `service_role` roles created to match Supabase's. Production was never touched. What this does not cover: Supabase's own security/performance advisors, and any Supabase-specific platform behaviour. Equivalent lint queries were run locally and are reported, but they are not the advisors themselves.
- **The baseline had to be reconstructed rather than replayed.** Applying the repository's migrations in order fails at the first file: `schema "relife" does not exist`, because the migration that creates the schema (`create_relife_chamber_core`) is applied in production but has no file in the repository. The baseline for this verification was therefore built by mirroring production's actual `organizations` and `clinics` definitions, read read-only. This is drift item 8 below, now demonstrated by execution instead of inferred.
