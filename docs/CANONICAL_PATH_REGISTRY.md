...
| Staff + finance configuration decision | `lib/domain/tenancy/staffFinanceConfiguration.ts` (Phase D application resolver; runtime readers/writers remain the existing staff and finance paths until their bounded integration lands) | Owner/settings authority for provisioning; existing finance permissions remain separate | Phase A staff tenant bindings and feature resolver; existing Sheets staff/finance ledgers remain operational authority |
| Owner active-clinic selection | `app/api/tenant/selection` → `lib/domain/tenancy/staffTenantContext.ts` / `tenantSelection.ts` → `relife-tenant-context` | authenticated active Owner membership; downstream action permissions remain separate | `relife.staff_tenant_bindings`; HttpOnly cookie is a revalidated preference, never an authorization authority |
| Platform Owner clinic provisioning + commercial control | `/platform` → `app/api/platform/clinics` → `lib/data/platformOwner.ts`; clinic creation delegates to existing `relife.provision_clinic_v1`; profile/owner changes reuse canonical config/staff provisioning; activation delegates to `relife.activate_clinic_v1` | signed session plus explicit server-side `PLATFORM_OWNER_STAFF_IDS` allowlist; Clinic Owner/System Admin tenant roles do not imply platform authority | Existing `relife.organizations`, `clinics`, tenant configuration/bindings, feature flags and bounded entitlements; activation remains exact-release readiness-gated and suspension never deletes tenant business data |
