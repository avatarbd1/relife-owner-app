...
test("hardened privileged tenant Edges require explicit organization and clinic slugs", () => {
  for (const path of privilegedTenantEdges) {
    ...
    assert.match(edge, /const organizationSlug\s*=\s*norm\(body\.organizationSlug\)/);
    assert.match(edge, /const clinicSlug\s*=\s*norm\(body\.clinicSlug\)/);
    assert.match(
      edge,
