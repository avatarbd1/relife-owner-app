...
function parsePending(
  ...
  return rows.slice(1).flatMap((row) => {
    ...
    if (normalized(at(row, idx("Status")) || "Pending") !== "pending") return [];
    ...
    const requestedAmount = money(
      at(row, idx("Requested_Amount")) || at(row, idx("Amount"))
    );
    return [{
      movementId,
...
export async function getPendingCashMovements(
  context: AccessContext,
  scope: Scope
): Promise<PendingCashMovement[]> {
  const departments = (["Physio", "Dental"] as ClinicDepartment[]).filter(
    (department) =>
      scopeAllows(scope, department) && canPerform(context, "cash.accept", department)
  );
  ...
  return groups.flat().sort((a, b) =>
    `${b.date}|${b.movementId}`.localeCompare(`${a.date}|${a.movementId}`)
  );
}
...
export async function finalizeCashMovement(
  ...
  const receivedAmount = decision === "Accepted"
    ? Number(input.receivedAmount ?? requestedAmount)
    : 0;
  ...
  if (receivedIdx >= 0) {
    requests.push(updateCellRequest(sheetId, rowNumber, receivedIdx + 1, receivedAmount));
  }
  if (differenceIdx >= 0) {
    requests.push(updateCellRequest(sheetId, rowNumber, differenceIdx + 1, difference));
  }
  if (updatedIdx >= 0) {
    requests.push(updateCellRequest(sheetId, rowNumber, updatedIdx + 1, now.timestamp));
  }
