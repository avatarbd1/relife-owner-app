...
function scoreRecord<T extends PatientSearchRecord>(patient: T, rawQuery: string): PatientSearchResult<T> | null {
  ...
  if (queryCompact && patientIdCompact === queryCompact) return { patient, score: 1000, matchedBy: "ID" };
  if (queryDigits.length >= 4 && phoneDigits === queryDigits) return { patient, score: 960, matchedBy: "Phone" };
  if (fullName === query) return { patient, score: 940, matchedBy: "Name" };
  if (queryCompact && patientIdCompact.startsWith(queryCompact)) return { patient, score: 900, matchedBy: "ID" };
  if (queryDigits.length >= 4 && phoneDigits.startsWith(queryDigits)) return { patient, score: 860, matchedBy: "Phone" };
  if (hasExactWord(fullName, query)) return { patient, score: 850, matchedBy: "Name" };
  if (queryDigits.length >= 4 && phoneDigits.endsWith(queryDigits)) return { patient, score: 840, matchedBy: "Phone" };
  if (fullName.startsWith(query)) return { patient, score: 820, matchedBy: "Name" };
  if (startsWord(fullName, query)) return { patient, score: 800, matchedBy: "Name" };
  if (includesAllTokens(fullName, queryTokens)) return { patient, score: 760, matchedBy: "Name" };
  if (queryDigits.length >= 4 && phoneDigits.includes(queryDigits)) return { patient, score: 730, matchedBy: "Phone" };
  if (patientId.includes(query) || patientIdCompact.includes(queryCompact)) return { patient, score: 700, matchedBy: "ID" };
  if (diagnosis.includes(query) || includesAllTokens(diagnosis, queryTokens)) return { patient, score: 520, matchedBy: "Diagnosis" };
  if (address.includes(query) || includesAllTokens(address, queryTokens)) return { patient, score: 500, matchedBy: "Address" };
  if (therapist.includes(query) || includesAllTokens(therapist, queryTokens)) return { patient, score: 480, matchedBy: "Therapist" };

  const combined = [patientId, fullName, normalizeSearchText(patient.phone), diagnosis, address, therapist].join(" ");
  if (includesAllTokens(combined, queryTokens)) return { patient, score: 420, matchedBy: null };
  return null;
}
