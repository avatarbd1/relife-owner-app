  ...
  return (
    <div>
      ...
      <Section title="Operations">
        {canChamber && (
          <ActionRow href="/chamber" icon="clinical" title="Live chamber" subtitle="4 beds, traction, machine availability and timers" />
        )}
        <ActionRow href="/daily" icon="attendance" title="Attendance" subtitle="Check in, break and check out" />
        {canReadRegister && (
          <ActionRow href="/register" icon="register" title="Daily register" subtitle="Department-safe operational register" />
        )}
        {canAcceptCash && !isOwner && (
          <ActionRow href="/finance/cash-receive" icon="cash" title="Receive cash handover" subtitle="Confirm actual amount received" />
        )}
        {canPhysioInventory && (
          <ActionRow href="/tools" icon="inventory" title="Inventory" subtitle="Authorized Physio stock and inventory log" />
        )}
      </Section>
      ...
      {(canPhysioClinicalTools || canCorrect) && (
        <Section title="Clinical & learning">
          {canPhysioClinicalTools && (
            <ActionRow href="/tools" icon="clinical" title="Clinical tools" subtitle="Clinical AI, Staff AI and case study" />
          )}
          {canCorrect && (
            <ActionRow href="/corrections" icon="correction" title="Same-day correction" subtitle="Eligible same-day entries with audit trail" />
          )}
        </Section>
      )}
      ...
      <Section title="Account & security">
        <ActionRow
          ...
          icon="security"
          title="Security"
          subtitle={isOwner ? "Access posture, devices and staff setup" : "Fingerprint, Face ID and device passkeys"}
        />
        {isOwner && (
          <ActionRow href="/security/staff-access" icon="staff" title="Staff web access" subtitle="Secure first-time setup links" />
        )}
      </Section>
      ...
      {(canAudit || canSettings) && (
        <Section title="Administration">
          {canAudit && (
            <ActionRow href="/audit" icon="history" title="Audit log" subtitle="Security, patient, finance and access timeline" />
          )}
          {canSettings && (
            <ActionRow href="/settings" icon="approval" title="Settings & administration" subtitle="Access matrix, data tools and system capability status" />
          )}
        </Section>
      )}
