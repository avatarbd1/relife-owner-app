...
export default async function FinanceRecordsPage() {
  ...
  const cards = [
    ...
    snapshot.capabilities.cash && ["Cash handover history", "Reception → Treasury → Bank movement and discrepancy evidence", "/finance/cash-history", "bg-sky-50 text-sky-900"],
    snapshot.capabilities.salary && ["Salary history", "Payroll and advance payment evidence by staff and month", "/finance/salary-history", "bg-teal-50 text-teal-900"],
    snapshot.capabilities.audit && ["Finance audit", "Owner/Auditor cross-ledger investigation view", "/finance/audit", "bg-slate-100 text-slate-900"],
  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-xl bg-gradient-to-br from-slate-950 to-blue-950 p-5 text-white shadow-lg">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-blue-200">Finance</p><h1 className="mt-1 text-2xl font-bold">Finance records</h1><p className="mt-1 text-xs leading-5 text-slate-300">Collections, expenses, cash handovers and payroll stay in separate ledgers. Every page is role + department scoped.</p></div><StatusBadge tone="info" className="border-white/10">Separated</StatusBadge></div>
      ...
      {context.roles.includes("Owner") && (
        <Link href="/finance/approvals" className="flex min-h-12 items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-4 text-sm font-semibold text-amber-900"><span>Pending approvals</span><span aria-hidden="true">→</span></Link>
      )}
    </div>
  );
}
