...
import { getTodaysCollection, getMonthBusinessPosition, getSalaryStatus } from "@/lib/calculations";
  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-xl bg-gradient-to-br from-blue-950 via-blue-900 to-slate-950 p-5 text-white shadow-lg">
        ...
        <div className="mt-5"><p className="text-xs text-blue-100/75">Today collected</p><p className="mt-1 text-3xl font-bold tabular-nums">{formatBDT(scopeCollection)}</p></div>
      </section>

      <ScopeSelector current={scope} allowed={allowedScopes} />

      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold text-slate-900">This month</h2><p className="mt-0.5 text-xs text-slate-500">Collection against business commitments</p></div><StatusBadge tone={uncovered ? "warning" : "success"}>{uncovered ? "Cost not recovered" : "Recovered"}</StatusBadge></div>
        <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-lg bg-blue-50 p-3"><p className="text-[11px] font-medium text-blue-700">Month collection</p><p className="mt-1 text-lg font-bold tabular-nums text-blue-950">{formatBDT(month.monthCollection)}</p></div><div className={`rounded-lg p-3 ${uncovered ? "bg-red-50" : "bg-emerald-50"}`}><p className={`text-[11px] font-medium ${uncovered ? "text-red-700" : "text-emerald-700"}`}>{uncovered ? "Still uncovered" : "Surplus"}</p><p className={`mt-1 text-lg font-bold tabular-nums ${uncovered ? "text-red-900" : "text-emerald-900"}`}>{formatBDT(Math.abs(month.surplusOrUncovered))}</p></div></div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold text-slate-900">Current cash position</h2><p className="mt-0.5 text-xs text-slate-500">Accepted movements and actual cash effects</p></div><span className="text-lg font-bold tabular-nums text-slate-950">{formatBDT(cash.total)}</span></div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">{[
          ["Payment", "Patient payment / receipt", "/payments", "bg-blue-50 text-blue-800"],
          ...
          ["Salary", "Staff payroll cards", "/salary", "bg-emerald-50 text-emerald-800"],
