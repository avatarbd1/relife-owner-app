...
export default async function FinancePage() {
  ...
  const advancedFinanceEnabled = await hasTenantFeature(
  ...
  const pendingTotal = advancedFinanceEnabled ? pendingExpenses.length + pendingCash.length : 0;
  const uncovered = month.surplusOrUncovered < 0;
  const workflows = [
    ["Collections", "Patient payment / receipt", "/payments", "bg-blue-50 text-blue-800"],
    ["Clinic Expense", "Request and record clinic expense", "/operations?tab=expenses", "bg-amber-50 text-amber-800"],
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">{workflows.map(([title, subtitle, href, tone]) => <Link key={String(title)} href={String(href)} className={`min-h-24 rounded-xl p-3 ring-1 ring-inset ring-slate-200/70 ${tone}`}><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-[11px] leading-4 opacity-80">{subtitle}</p></Link>)}</div>
