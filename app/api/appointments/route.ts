...
function errorResponse(error: unknown): NextResponse {
  ...
  if (message === "APPOINTMENT_DUPLICATE") return NextResponse.json({ ok: false, error: message }, { status: 409 });
  if (message.startsWith("APPOINTMENT_CONFLICT:")) {
    const [, type, ...detailParts] = message.split(":");
    return NextResponse.json({ ok: false, error: "APPOINTMENT_CONFLICT", conflictType: type || "other", detail: detailParts.join(":") || "Booking conflict", validation: typed.validation }, { status: 409 });
  }
  if (message.startsWith("APPOINTMENT_CAPACITY:")) return NextResponse.json({ ok: false, error: "APPOINTMENT_CAPACITY", detail: message.split(":").slice(1).join(":") }, { status: 409 });
  if (["INVALID_DATE", "INVALID_TIME", "INVALID_SLOT", "INVALID_THERAPIST", "INVALID_REQUEST_ID"].includes(message)) return NextResponse.json({ ok: false, error: message }, { status: 400 });
  if (["SCHEMA_MISMATCH", "SUPABASE_EDGE_SECRET_MISSING", "TENANT_NOT_FOUND"].includes(message)) return NextResponse.json({ ok: false, error: message }, { status: 503 });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!patient || patient.department === "All") return NextResponse.json({ ok: false, error: "PATIENT_NOT_FOUND" }, { status: 404 });
