...
function planForAppointment(appointment: BoardAppointment) {
  ...
  if (cursor < 60) steps.push({ name: selected.length ? "Therapist time" : "Therapist session", value: "THERAPIST-TIME", resourceId: "", durationMin: 60 - cursor, startOffset: cursor, endOffset: 60 });
  return steps;
}
...
export default function ChamberHourlyBedBoard({ date, today, appointments, slots, patients, clinicians, canBook }: { date: string; today: string; appointments: BoardAppointment[]; slots: Slot[]; patients: PatientOption[]; clinicians: ClinicianOption[]; canBook: boolean }) {
  ...
  function closeBooking() { setOpenSlot(null); setPatientText(""); setTherapist(""); setModalities([]); setRemarks(""); setValidation(null); setSuggestionsApplied(false); setError(""); }
  ...
  async function createBooking() {
    ...
    try { const response = await fetch("/api/chamber/fixed-hour", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "create", patientId: selectedPatient.patientId, date, time: openSlot.time, therapist, modalities, remarks, requestedBedId: openSlot.bedId }) }); const result = await response.json().catch(() => ({})); if (!response.ok || !result.ok) { const detail = result.validation?.conflicts?.map((item: { message: string }) => item.message).join(" · "); throw new Error(detail || result.detail || result.error || "Booking failed"); } haptic("success"); closeBooking(); router.refresh(); }
