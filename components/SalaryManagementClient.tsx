      ...
      {tab === "summary" && (
        <div className="relife-fade-in space-y-4">
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {excessPaid > 0 && <div className="mt-3"><InlineNotice tone="warning">Ledger paid exceeds this month’s fixed commitment by {bdt(excessPaid)}. The current 13_Salary source has no Salary/Advance type field, so the excess is not auto-classified.</InlineNotice></div>}
          </section>
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-slate-950/50 p-4 backdrop-blur-[1px] sm:items-center sm:justify-center" role="dialog" aria-modal="true" aria-label="Pay salary">
          <div className="relife-page-enter w-full max-w-md rounded-2xl bg-white p-4 shadow-xl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div><label className="mb-1.5 block text-xs font-semibold text-slate-700">Note</label><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Payment note (type is not classified)" className="min-h-11 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-blue-800 focus:ring-2 focus:ring-blue-100" /></div>
