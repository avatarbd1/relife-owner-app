      ...
      <section className="rounded-2xl bg-slate-900 p-4 text-white shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-300">
              {self?.checkIn
                ? `In ${self.checkIn}`
                : "আজকের attendance শুরু হয়নি"}
              {self?.checkOut ? ` · Out ${self.checkOut}` : ""}
            </p>
          </div>
          {self?.workingHours !== null && self?.workingHours !== undefined && (
            <span className="rounded-xl bg-white/10 px-3 py-2 text-sm font-semibold">
              {self.workingHours}h
            </span>
          )}
        </div>
        ...
        {!self?.checkIn && (
          <p className="mt-3 text-[11px] text-slate-400">
            Normal attendance · GPS/location প্রয়োজন নেই
          </p>
        )}
        ...
        {message && (
          <p
            className={`mt-3 text-xs ${
              message.startsWith("✓") ? "text-emerald-300" : "text-red-300"
            }`}
            role="status"
          >
            {message}
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-slate-500">{snapshot.date}</p>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          ...
          <div className="rounded-xl bg-sky-50 p-3">
            ...
            <p className="text-[10px] text-sky-600">Appointments</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-slate-400">
              {snapshot.appointmentCounts.total} total
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 space-y-2">
          {snapshot.appointments.slice(0, 40).map((item) => (
            <div
              ...
              className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5"
            >
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <div className="flex items-end justify-between gap-3">
          ...
          <span className="text-xs text-slate-500">
            {snapshot.attendance.team.length}
          </span>
        </div>
        <div className="mt-3 space-y-2">
          {snapshot.attendance.team.map((staff) => (
            <div
              key={staff.staffId}
              className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-slate-100 px-3 py-2.5"
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-900">
                  {staff.fullName}
                ...
                <p className="truncate text-[11px] text-slate-500">
                  ...
                  {staff.checkIn ? ` · In ${staff.checkIn}` : ""}
                </p>
              </div>
              <span
                className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold ${stateClass(
                  staff.state
                )}`}
              >
